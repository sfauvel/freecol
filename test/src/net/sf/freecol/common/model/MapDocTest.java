...
public class MapDocTest extends FreeColGuiDocAsTest {
    ...
    public void testMapGameInt() throws FreeColException {
        ...
        write("",
                getFormatter().sourceCode(
                        CodeExtractor.extractPartOfCurrentMethod()
                ),
                "",
                String.format("Dimension of the building map is %dx%d.",
...
//        assertTrue(surroundingTiles.contains(map.getTile(4, 9)));

        {
            final int range = 1;
            final Tile tile = map.getTile(4, 8);
            write(String.format("Get surrounding tile of the tile %s with a range %d",
                            tilePosition.apply(tile), range),
                    "are",
                    StreamSupport.stream(tile.getSurroundingTiles(range).spliterator(), false)
                            .map(tilePosition)
...
//        assertEquals(25 - 1, surroundingTiles.size());

        {
            ...
            write(String.format("Get surrounding tile of the tile %s with a range %d",
                            tilePosition.apply(tile), range),
                    String.format("there is %d tiles.",
                            StreamSupport.stream(tile.getSurroundingTiles(range).spliterator(), false).count()),
                    "", "");
        }
...
//
    ...
    public void testGetReverseDirection() {
        write("[%autowidth]",
                getFormatter().table(
                        Stream.concat(
                                Stream.of(Arrays.asList("Direction", "Reverse direction")),
                                Arrays.stream(Direction.values())
                                        .map(d -> Arrays.asList(d, d.getReverseDirection()))
