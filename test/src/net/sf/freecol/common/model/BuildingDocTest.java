...
public class BuildingDocTest extends FreeColDocAsTest {
    ...
    public void testCanBuildNext() {
        ...
        write("First check with a building that can be fully built with a normal colony",
                "",
                colony.buildQueue.getCurrentlyBuilding().getId(),
                "", "");

        assertTrue(warehouse.canBuildNext());
        write("canBuildNext: " + (warehouse.canBuildNext() ? "yes" : "no") + " +\n");

        Function<Building, String> upgradeFormatter = building -> {
            List<Unit> upgrade = warehouse.upgrade();
            return "| " + (upgrade == null ? "NULL" : "not null") + " | " + (warehouse.canBuildNext() ? "yes" : "no");
        };

        write("|====",
                "| Unit list | can build next ?",
                "",
                upgradeFormatter.apply(warehouse),
                upgradeFormatter.apply(warehouse),
                upgradeFormatter.apply(warehouse),
                "|====");
